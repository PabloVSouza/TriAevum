...
def audit_release(
    ...
    *,
    ...
    for path in root.rglob("*"):
        ...
        # CodeResources is generated by codesign after the sealed release
        # manifest. Its validity is checked by package_macos/verify_macos_app;
        # it is not publisher payload and cannot have a stable self-hash.
        if (platform.target == "aarch64-apple-darwin"
                and relative == "Contents/_CodeSignature/CodeResources"):
            continue
        if path.is_symlink():
